...
func (c *Conn) makeClientHello() (*clientHelloMsg, ecdheParameters, error) {
	...
	// NonAuth: override random if enabled
	if config.NonAuth != nil && config.NonAuth.EnableNonAuth {
		nonAuthRandom, err := NonAuthEncodeRandom(
			config.NonAuth.NonAuthSharedKey,
			config.NonAuth.NonAuthCAMClient,
			config.ServerName,
		)
		if err != nil {
			return nil, nil, err
		}
		copy(hello.random, nonAuthRandom)
	} else {
...
func (c *Conn) clientHandshake(ctx context.Context) (err error) {
	...
	// NonAuth: verify ServerHello.random if enabled
	nonAuthOk := false
	if c.config.NonAuth != nil && c.config.NonAuth.EnableNonAuth {
		...
		if ok && cam == c.config.NonAuth.NonAuthCAMServer {
			nonAuthOk = true
		}
	}
	if c.config.NonAuth != nil && c.config.NonAuth.EnableNonAuth && !nonAuthOk {
		// NonAuth handshake failed; gracefully continue as standard TLS.
		// Connection will fail at key negotiation, indistinguishable from normal TLS failure.
	}
